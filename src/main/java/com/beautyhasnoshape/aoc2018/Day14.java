 ...
 * --- Day 14: Chocolate Charts ---
 ...
 nutmeg, vanilla... the Elves must be growing these plants to make hot chocolate!
 As you realize this, you hear a conversation in the distance. When you go to investigate, you discover two Elves
 ...
 So, after the first round, the first Elf moves forward 1 + 3 = 4 times, while the second Elf moves forward
 1 + 7 = 8 times.
 If they run out of recipes, they loop back around to the beginning.
 After the first round, both Elves happen to loop around until they land on the same recipe that they had
 in the beginning; in general, they will move to different recipes.
 ...
 However, that could take ages; you can speed this up considerably by identifying the scores of the ten recipes
 ...
 Your puzzle answer was 6548103910.
 ...
 Your puzzle answer was 20198090.
 */
public class Day14 {

    public String solvePartA(String input, int start) {
        byte[] recipes = new byte[start + 12];

        int i = 0, idxA = 0, idxB = 1, recipesCount = input.length(), stop = start + 10;
        for (byte aByte : input.getBytes()) {
            recipes[i++] = (byte) (aByte - '0');
        }

        byte recipeA = recipes[idxA];
        byte recipeB = recipes[idxB];

        do {
            String newRecipes = "" + (recipeA + recipeB);

            for (byte aByte : newRecipes.getBytes()) {
                recipes[recipesCount++] = (byte) (aByte - '0');
            }

            idxA = (idxA + recipeA + 1) % recipesCount; // shift right by recipes[idxA] + 1
            idxB = (idxB + recipeB + 1) % recipesCount;

            recipeA = recipes[idxA];
            recipeB = recipes[idxB];
        } while (recipesCount < stop + 2);
    ...
    public String solvePartB(String input, int[] toFind) {
        byte[] recipes = new byte[Integer.MAX_VALUE / 100];

        int i = 0, idxA = 0, idxB = 1, recipesCount = input.length();
        for (byte aByte : input.getBytes()) {
            recipes[i++] = (byte) (aByte - '0');
        }
        ...
        do {
            ...
            for (byte aByte : newRecipes.getBytes()) {
                ...
                // check against searched value
                if (recipesCount >= toFind.length) {
                    isFound = true;
                    for (int j = 0; j < toFind.length; j++) {
                        if (recipes[recipesCount - toFind.length + j] != toFind[j]) {
                            isFound = false;
                        }
                    }
                    if (isFound) {
        ...
        return "" + (recipesCount - toFind.length);
    }
}
