...
/**
 ...
 that there is one marble between the marble that was just placed and the current marble.)
 ...
 (because player 5 kept marble 23 and removed marble 9, while no other player got any points in this
...
public class Day09 {
    ...
    public long solvePartA(int playersCount, int lastMarbleNo) {
        return solve(playersCount, lastMarbleNo);
    }

    public long solvePartB(int playersCount, int lastMarbleNo) {
        return solve(playersCount, lastMarbleNo);
    }

    public long solve(int playersCount, int lastMarbleNo) {
        int currentPlayer = 0;
        long[] scores = new long[playersCount]; // idx = playerNo, values = player's score
        Circle circle = new Circle();

        for (int marble = 0; marble <= lastMarbleNo; marble++) {
            if (!circle.isEmpty() && marble % 23 == 0) {
                circle.move(-7);
                Circle.Element removedMarble = circle.remove();

                scores[currentPlayer] += marble;
                scores[currentPlayer] += removedMarble.value;
            } else {
                circle.move();
                circle.add(marble);
            }
