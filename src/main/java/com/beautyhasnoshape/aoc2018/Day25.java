...
/**
 ...
 "Just a moment, I think I can remotely run a diagnostic scan." A beam of light projects from the device
 and sweeps over the reindeer a few times.
 ...
 and send some hot chocolate back to you. You'll need a list of fixed points in spacetime;
 ...
 to the destination and open the portal. Let me look up how much hot chocolate that breed of reindeer needs."
 ...
 between the two of them. (That is, if a point is close enough to a constellation, it "joins" that constellation.)
 For example:
  0, 0, 0, 0
  3, 0, 0, 0
  0, 3, 0, 0
 ...
 and the point at 0,0,0,6 is connected to the others by being 3 away from 0,0,0,3, which is already
 in the constellation. The bottom two points, 9,0,0,0 and 12,0,0,0 are in a separate constellation because no point
 is close enough to connect them to the first constellation. So, in the above list, the number of constellations is 2.
 (If a point at 6,0,0,0 were present, it would connect 3,0,0,0 and 9,0,0,0, merging all of the points
...
public class Day25 {
    public static class Point implements Comparable<Point> {
        ...
        public boolean isInRange(Point other) {
    ...
    public int solvePartA(List<String> lines) {
        List<List<Point>> constellations = new ArrayList<>(1000);
        List<Point> points = convertToPoints(lines);

        for (Point point : points) {
            List<List<Point>> constellationsInRange = new ArrayList<>(10);
            for (List<Point> constellation : constellations) {
                if (isInRange(point, constellation)) {
                    constellation.add(point);
                    constellationsInRange.add(constellation);
                }
            }

            if (constellationsInRange.isEmpty()) {
                List<Point> newConstellation = new ArrayList<>();
                newConstellation.add(point);
                constellations.add(newConstellation);
            } else {
                // add to the first constellation
                constellationsInRange.get(0).add(point);
                if (constellationsInRange.size() > 1) {
                    for (int i = 1; i < constellationsInRange.size(); i++) {
                        constellationsInRange.get(0).addAll(constellationsInRange.get(i));
                        constellations.remove(constellationsInRange.get(i));
                    }
                }
            }
        }
    ...
    private List<Day25.Point> convertToPoints(List<String> lines) {
    ...
    private boolean isInRange(Point point, List<Point> constallation) {
        for (Point p : constallation) {
            if (point.isInRange(p)) {
                return true;
            }
        }
